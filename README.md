<h1 align="center">JS Recon — Terraform DigitalOcean Module</h1>

<p align="center">
  Provision a DigitalOcean Droplet that runs <a href="https://js-recon.io">JS Recon</a> against any URL.
  Surface exposed endpoints, client-side vulnerabilities, and leaked source maps automatically.
</p>

<p align="center">
  <a href="https://score.getplumber.io/github.com/js-recon/terraform-digitalocean-js-recon">
    <img src="https://score.getplumber.io/github.com/js-recon/terraform-digitalocean-js-recon.svg" alt="Plumber Score">
  </a>
</p>
...
Set your DigitalOcean token before applying:
...
The scan runs automatically when the Droplet starts. Destroy the Droplet after the scan:
...
Logs are written to `/var/log/js-recon.log`.
...
| `tags` | No | `[]` | Tags to apply to the Droplet |
...
JS Recon writes the following files and uploads them to Spaces:
...
Artifacts are uploaded to `s3://<bucket>/<prefix>/` via the AWS CLI using the DigitalOcean Spaces S3-compatible endpoint.
...
With a schedule, the Droplet stays running and executes the scan via cron. Without a schedule, the Droplet runs the scan once at creation and remains idle until destroyed.

---

## Spaces access keys

Generate Spaces access keys at **DigitalOcean → API → Spaces Keys**. Pass them as variables or via environment:
